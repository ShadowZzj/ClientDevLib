            ...
            if (duration.count() >= 500)
            {
                ...
                if (itemContainer != nullptr && itemContainer->dropItemPtr)
                {
                    ...
                    for (GameManager::DropItem *item = itemContainer->dropItemPtr; item != nullptr; item = item->next)
                    {
                        ...
                        if (!item->canPick)
                            continue;
                        auto distance = sqrt(pow(item->x - localPlayer->x, 2) + pow(item->y - localPlayer->y, 2) + pow(item->z-localPlayer->z,2));
                        if (distance < 4)
                        {
...
void GameSetting::Render(bool &open)
{
    ...
    std::string localPlayerPosPrint =
        fmt::format("localPlayer pos {:.2f},{:.2f},{:.2f} hp:{} mp:{}", localPlayer->x, localPlayer->y, localPlayer->z,localPlayer->GetCurrentHP(),localPlayer->GetCurrentMP());
    ...
    for (auto &player : aroundPlayers)
    {
        ...
        {
            std::string textStr =
                fmt::format("{} found!   position {:.2f},{:.2f},{:.2f} distance {:.2f}", name, player.x, player.y, player.z,
    ...
    aroundPlayersExceptMine.erase(std::remove_if(aroundPlayersExceptMine.begin(), aroundPlayersExceptMine.end(),
                       [&](GameManager::CLocalUser &player){
                        ...
                        for(auto & [name, roleConfig] : gameManager.config["roleConfig"].items())
                        {
                            if(playerName == name)
                                return true;
                        }
