...
std::string GUIDToString(GUID guid)
{
    std::string guidStr;
    guidStr =
        fmt::format("{:08X}-{:04X}-{:04X}-{:02X}{:02X}-{:02X}{:02X}{:02X}{:02X}{:02X}{:02X}",
...
bool zzj::Domain::IsDomainJoined()
{
    ...
    DWORD dwErr =
        DsRoleGetPrimaryDomainInformation(NULL, DsRolePrimaryDomainInfoBasic, (PBYTE *)&pdsrib);
    if (dwErr != ERROR_SUCCESS)
    {
        throw std::runtime_error(
            fmt::format("DsRoleGetPrimaryDomainInformation failed with error code {}", dwErr));
    }
...
zzj::Domain zzj::Domain::GetDomain()
{
    if (IsDomainJoined())
    {
        ...
        if (dwErr == ERROR_SUCCESS)
        {
            ...
            std::string strippedDCName =
                domainControllerName.substr(2, domainControllerName.find_first_of('.') - 2);
