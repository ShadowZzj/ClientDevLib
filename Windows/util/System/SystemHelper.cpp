...
bool zzj::SystemInfo::IsWindowsVersionGreaterOrEqual(WORD wMajorVersion, WORD wMinorVersion,
...
bool zzj::SystemInfo::IsWindowsVersionLessOrEqual(WORD wMajorVersion, WORD wMinorVersion,
                                                  WORD wServicePackMajor)
{
    OSVERSIONINFOEXW osvi = {sizeof(osvi), 0, 0, 0, 0, {0}, 0, 0};
    DWORDLONG const dwlConditionMask = VerSetConditionMask(
        VerSetConditionMask(0, VER_MAJORVERSION, VER_EQUAL), VER_MINORVERSION, VER_LESS_EQUAL);
...
{
    return IsWindowsVersionGreaterOrEqual(HIBYTE(_WIN32_WINNT_WINXP), LOBYTE(_WIN32_WINNT_WINXP),
                                          0);
}
...
{
    return IsWindowsVersionGreaterOrEqual(HIBYTE(_WIN32_WINNT_VISTA), LOBYTE(_WIN32_WINNT_VISTA),
                                          0);
}
...
{
    return IsWindowsVersionGreaterOrEqual(HIBYTE(_WIN32_WINNT_WINBLUE),
                                          LOBYTE(_WIN32_WINNT_WINBLUE), 0);
}
bool zzj::SystemInfo::IsWindows10OrGreater()
{
    return IsWindowsVersionGreaterOrEqual(HIBYTE(_WIN32_WINNT_WIN10), LOBYTE(_WIN32_WINNT_WIN10),
                                          0);
}

bool zzj::SystemInfo::IsWindows11OrGreater()
{
    auto version = GetWindowsVersion();
    if (version.has_value())
    {
        // Windows 11 has major version 10 and build number >= 22000
        if (version->major == 10 && (version->buildnumber & 0xffff) >= 22000)
        {
            return true;  // Windows 11 or greater
        }
        else if (version->major > 10)
        {
            return true;  // Future versions beyond Windows 11
        }
    }
    return false;  // Windows 10 or earlier
}
...
std::optional<std::string> zzj::SystemInfo::GetComputerNameStr()
{
    ...
    if (bStatus) return zzj::str::ansi2utf8(szComputerName);
    return {};
}
