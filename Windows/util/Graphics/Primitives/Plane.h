...
{
  public:
    template <class V> static IndexedTriangleList<V> MakeTesselated(int divisions_x, int divisions_y)
    {
        ...
        indices.reserve(sq(divisions_x * divisions_y) * 6);
        {
            ...
            for (size_t y = 0; y < divisions_y; y++)
            {
                ...
                {
                    const std::array<unsigned short, 4> indexArray = {vxy2i(x, y), vxy2i(x + 1, y), vxy2i(x, y + 1),
                                                                      vxy2i(x + 1, y + 1)};
