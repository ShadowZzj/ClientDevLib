      ...
      dchi(odist(rng)), chi(adist(rng)), theta(adist(rng)), phi(adist(rng))
{
    ...
    if (!IsStaticInitialized())
    {
        ...
        auto pvs = std::make_unique<VertexShader>(gfx, (currentExecutablePath / "ColorIndexVS.cso").wstring().c_str());
        auto pvsbc = pvs->GetBytecode();
        AddStaticBind(std::move(pvs));

        AddStaticBind(
            std::make_unique<PixelShader>(gfx, (currentExecutablePath / "ColorIndexPS.cso").wstring().c_str()));
