    ...
    : TestObject(gfx, rng, adist, ddist, odist, rdist)
{
    ...
    boost::filesystem::path currentExecutablePath = zzj::GetExecutablePath();
    if (!IsStaticInitialized())
    {
        ...
        auto pvs   = std::make_unique<VertexShader>(gfx, (currentExecutablePath / "PhongVS.cso").wstring().c_str());
        auto pvsbc = pvs->GetBytecode();
        AddStaticBind(std::move(pvs));

        AddStaticBind(std::make_unique<PixelShader>(gfx, (currentExecutablePath / "PhongPS.cso").wstring().c_str()));
...
bool Box::SpawnControlWindow(int id, Graphics &gfx) noexcept
{
    ...
    if (ImGui::Begin(("Box "s + std::to_string(id)).c_str(), &open))
    {
        ...
        const auto sid =
            ImGui::SliderFloat("Specular Intensity", &materialConstants.specularIntensity, 0.05f, 4.0f, "%.2f", 2);
        const auto spd =
            ImGui::SliderFloat("Specular Power", &materialConstants.specularPower, 1.0f, 200.0f, "%.2f", 2);
        dirty = cd || sid || spd;
