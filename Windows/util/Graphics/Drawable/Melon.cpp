      ...
      dchi(odist(rng)), chi(adist(rng)), theta(adist(rng)), phi(adist(rng))
{
    ...
    {
        auto pvs   = std::make_unique<VertexShader>(gfx, (currentExePath / "ColorIndexVS.cso").wstring().c_str());
        auto pvsbc = pvs->GetBytecode();
        AddStaticBind(std::move(pvs));

        AddStaticBind(std::make_unique<PixelShader>(gfx, (currentExePath / "ColorIndexPS.cso").wstring().c_str()));
