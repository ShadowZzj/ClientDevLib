...
ResultType convert_property_data(const event_property&)
{
...
ResultType convert_property_data(const event_property& prop)
{
	if (prop.get_in_type() == EventType::tdh_type)
	{
		...
		if (prop.get_raw_value().size() != sizeof(EventType::type))
			throw event_trace_error("Invalid property value size");

		return static_cast<ResultType>(*reinterpret_cast<const EventType::type*>(prop.get_raw_value().data()));
	}
