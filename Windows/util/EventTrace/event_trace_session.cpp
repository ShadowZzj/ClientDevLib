#include "event_trace_session.h"

#include <cassert>
#include <limits>

#include "guid_helpers.h"
#include "event_trace_error.h"
#include "event_trace_session_properties.h"
	...
	trace_level level, std::uint64_t match_any_keywords)
{
	...
	provider_guid_.insert({provider_guid,level});
}
...
void event_trace_session::disable_trace(const ms_guid& provider_guid)
{

    if (auto iter = std::find_if(provider_guid_.begin(), provider_guid_.end(),
                                 [&](const auto &it) { return it.first == provider_guid; });
        iter != provider_guid_.end())
    {
        auto result = ::EnableTraceEx2(handle_, provider_guid.native_ptr(), EVENT_CONTROL_CODE_DISABLE_PROVIDER,
                                       static_cast<UCHAR>(iter->second), 0, 0, 0, NULL);
        if (ERROR_SUCCESS != result)
            throw event_trace_error("Unable to enable trace for provider", result);
    }
}
