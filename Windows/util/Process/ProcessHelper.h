...
class Process
{
  ...
  public:
    ...
    static DWORD SystemCreateProcess(const std::wstring &commandLine, bool bElevated, bool bWait, DWORD dwWaitTime,
                                     bool show);
    // Same level as caller.
    static BOOL RegularCreateProcess(std::string path, bool show, std::string cmdLine = "", bool wait = false,
                                     DWORD *errCode = nullptr);
...
class Memory
{
  public:
    ...
    std::vector<MemoryInfo> PatternScan(uintptr_t startAddress, size_t searchSize, const std::string &pattern);
