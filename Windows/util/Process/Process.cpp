...
namespace zzj
{
    ...
    std::vector<ProcessV2::Module::Section> ProcessV2::Module::GetSections(uint32_t pid)
    {
        ...
        ReadProcessMemory(handle, (LPVOID)((std::uintptr_t)hModule + dosHeader.e_lfanew), &ntHeader, sizeof(ntHeader), NULL);

        DWORD sectionAlignment = ntHeader.OptionalHeader.SectionAlignment;
        IMAGE_SECTION_HEADER sectionHeader;
        for (int i = 0; i < ntHeader.FileHeader.NumberOfSections; i++)
        {
            ReadProcessMemory(handle, (LPVOID)((std::uintptr_t)hModule + dosHeader.e_lfanew + sizeof(ntHeader) + i * sizeof(sectionHeader)),
