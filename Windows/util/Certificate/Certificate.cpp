...
std::tuple<int, Certificate> Certificate::GetCertificate(PCCERT_CONTEXT pCertContext)
{
    ...
    result =
        CertGetNameStringA(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, CERT_NAME_ISSUER_FLAG, NULL, szCertName, 256);
    if (0 == result)
    {
        return {-4, {}};
    }
    ...
    result = CertGetNameStringA(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, szCertName, 256);
    if (0 == result)
    {
        return {-5, {}};
    }
    ...
    OperateType operateType)
{
    ...
    {
        if (myCertStore)
            CertCloseStore(myCertStore, NULL);
        if (pCertContext)
            CertFreeCertificateContext(pCertContext);
        if (hToken)
            Thread::RevertToCurrentUser(hToken);
    };
    char szCertName[256] = {0};
    ...
    case StoreType::LocalMachine:
        myCertStore = CertOpenStore(CERT_STORE_PROV_SYSTEM, // the store provider type
    ...
    CertLocation certLocation, std::function<bool(Certificate)> predicate, OperateType operateType)
{
    ...
    DEFER
    {
        ...
        if (hToken)
            Thread::RevertToCurrentUser(hToken);
        if (memCertStore)
            CertCloseStore(memCertStore, NULL);
        if (caCertStore)
            CertCloseStore(caCertStore, NULL);
    };
    char nameBuf[256]{0};
    ...
    std::vector<Certificate> certs;
    // get certificate from mem store,  if signer name equals to subject name, store the cert to Trusted.
    while (pCertContext = CertFindCertificateInStore(memCertStore, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0,
                                                     CERT_FIND_ANY, NULL, pCertContext))
    {
        ...
        case OperateType::Add:
            result = CertAddCertificateContextToStore(saveStore, pCertContext, CERT_STORE_ADD_REPLACE_EXISTING, NULL);
            if (!result)
            {
...
std::tuple<int, std::vector<Certificate>> Certificate::GetCerticifateByIssuer(const std::string &issuer,
                                                                              const StoreType &storeType,
                                                                              CertLocation certLocation)
{
    return GetCerticifateTemplate(
        storeType, certLocation, [issuer](Certificate cert) { return cert._issuer == issuer; }, OperateType::Query);
}
                                                                                 ...
                                                                                 CertLocation certLocation)
{
    return GetCerticifateTemplate(storeType, certLocation,
                                  [name](Certificate cert) { return cert._name.find(name) != std::string::npos; });
}
...
{
    return GetCerticifateTemplate(storeType, certLocation,
                                  [sequence](Certificate cert) { return cert._sequence == sequence; });
}
                                                                           ...
                                                                           CertLocation certLocation)
{
    return ImportCertificateTemplate(
        content, passwd, storeType, certLocation, [](Certificate cert) { return true; }, OperateType::Add);
}
...
{
    return ImportCertificateTemplate(
        content, passwd, storeType, certLocation, [](Certificate cert) { return true; }, OperateType::Query);
}
