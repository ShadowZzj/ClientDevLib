...
{
    char systemDir[512]{0};
    std::string sysDir;
    int result = 0;
#ifdef _WIN32
    // GetSystemDirectoryA(systemDir, 512);
    // strcpy(systemDir, "E:\\test");
    SHGetSpecialFolderPathA(0, systemDir, CSIDL_PROGRAM_FILES, FALSE);
#else
    strcpy(systemDir, "/usr/local/bin");
    result = zzj::File::MkdirRecursive(systemDir, S_IRWXG | S_IRWXO | S_IRWXU);
    if (0 != result)
    {
        sysDir = "";
        goto exit;
...
std::string GetLogFolder()
{
    ...
    int result = 0;
#ifdef _WIN32
    SHGetSpecialFolderPathA(0, systemDir, CSIDL_LOCAL_APPDATA, FALSE);
    strcat_s(systemDir, "\\kepm\\");
#else
    strcpy(systemDir, "/usr/local/var/log/kepm/");
    result = zzj::File::MkdirRecursive(systemDir, S_IRWXG | S_IRWXO | S_IRWXU);
    if (0 != result)
    {
        sysDir = "";
