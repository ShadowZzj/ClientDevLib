...
class ConfigBase
{
  public:
    ...
    template <typename T> ConfigBase &operator[](T key);

    template <typename T> void operator=(T dst);
...
template <typename T> ConfigBase &ConfigBase::operator[](T key)
{
    m_key.append(key);
    m_key.append(":");
    return *this;
...
template <typename T> void ConfigBase::operator=(T dst)
{
    m_json[m_key] = dst;
