...
//
// If needed:
// auto ValidateInjectionFunctions	= (f_ValidateInjectionFunctions)GetProcAddress(hInjectionMod,
// "ValidateInjectionFunctions"); auto RestorenjectionFunctions		=
// (f_RestoreInjectionFunctions)GetProcAddress(hInjectionMod, "RestorenjectionFunctions");
//
// Symbol state:
...
struct INJECTIONDATAW
{
    ...
    wchar_t szTargetProcessExeFileName[MAX_PATH]; // exe name of the target process, this value gets set automatically
                                                  // and should be initialized with 0s
    DWORD ProcessID;
    INJECTION_MODE Mode;
...
// ValidateInjectionFunctions fills an std::vector with this info, result can simply be passed to
...
// Cloaking options:
#define INJ_ERASE_HEADER                                                                                               \
    0x0001 // replaces the first 0x1000 bytes of the dll with 0's (takes priority over INJ_FAKE_HEADER if both are
           // specified)
#define INJ_FAKE_HEADER                                                                                                \
    0x0002 // replaces the dlls header with the header of the ntdll.dll (superseded by INJ_ERASE_HEADER if both are
           // specified)
#define INJ_UNLINK_FROM_PEB 0x0004 // unlinks the module from the process enviroment block (1)
#define INJ_THREAD_CREATE_CLOAKED                                                                                      \
    0x0008 // passes certain flags to NtCreateThreadEx to make the thread creation more stealthy (2)
...
#define INJ_MM_CLEAN_DATA_DIR                                                                                          \
    0x00010000 // removes data from the dlls PE header, ignored if INJ_MM_SET_PAGE_PROTECTIONS is set
...
#define INJ_MM_SET_PAGE_PROTECTIONS                                                                                    \
    0x00200000 // sets page protections based on section characteristics, if set INJ_MM_CLEAN_DATA_DIR will be ignored
...
#define MM_DEFAULT                                                                                                     \
    (INJ_MM_RESOLVE_IMPORTS | INJ_MM_RESOLVE_DELAY_IMPORTS | INJ_MM_INIT_SECURITY_COOKIE | INJ_MM_EXECUTE_TLS |        \
     INJ_MM_ENABLE_EXCEPTIONS | INJ_MM_RUN_DLL_MAIN | INJ_MM_SET_PAGE_PROTECTIONS)
...
using f_ValidateInjectionFunctions = bool(__stdcall *)(DWORD dwTargetProcessId, DWORD &ErrorCode, DWORD &LastWin32Error,
                                                       HookInfo *HookDataOut, UINT Count, UINT *CountOut);
using f_RestoreInjectionFunctions  = bool(__stdcall *)(DWORD dwTargetProcessId, DWORD &ErrorCode, DWORD &LastWin32Error,
                                                      HookInfo *HookDataIn, UINT Count, UINT *CountOut);
...
class GHInJector
{
  public:
	...
	bool Init()
    {
        hInjectionMod = LoadLibraryA("GHInjectorLibrary.dll");
        if (hInjectionMod == INVALID_HANDLE_VALUE)
            return false;
        InjectA               = (f_InjectA)GetProcAddress(hInjectionMod, "InjectA");
        if (NULL == InjectA)
            return false;
        GetSymbolState        = (f_GetSymbolState)GetProcAddress(hInjectionMod, "GetSymbolState");
        if (NULL == GetSymbolState)
            return false;
        GetImportState        = (f_GetSymbolState)GetProcAddress(hInjectionMod, "GetImportState");
        if (NULL == GetImportState)
            return false;
        StartDownload         = (f_StartDownload)GetProcAddress(hInjectionMod, "StartDownload");
        if (NULL == StartDownload)
            return false;
        GetDownloadProgressEx = (f_GetDownloadProgressEx)GetProcAddress(hInjectionMod, "GetDownloadProgressEx");
        if (NULL == GetDownloadProgressEx)
            return false;
    ...
    f_GetSymbolState GetImportState;
    f_StartDownload StartDownload;
