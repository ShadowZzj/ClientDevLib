...
{
    unsigned char *inPadded;
    unsigned int inPaddedLen;

    if (inLen % blockBytesLen != 0)
    {
        // zero padding
        inPadded = new unsigned char[inLen + blockBytesLen - inLen % blockBytesLen];
        memcpy(inPadded, in, inLen);
        memset(inPadded + inLen, 0, blockBytesLen - inLen % blockBytesLen);
        inPaddedLen = inLen + blockBytesLen - inLen % blockBytesLen;
    }
    else
    {
        inPadded = new unsigned char[inLen];
        memcpy(inPadded, in, inLen);
        inPaddedLen = inLen;
    }

    unsigned char *out       = new unsigned char[inPaddedLen];
    unsigned char *roundKeys = new unsigned char[4 * Nb * (Nr + 1)];
    KeyExpansion(key, roundKeys);
    for (unsigned int i = 0; i < inPaddedLen; i += blockBytesLen)
    {
        EncryptBlock(inPadded + i, out + i, roundKeys);
    }
...
unsigned char *AES::EncryptCBC(const unsigned char in[], unsigned int inLen, const unsigned char key[],
                               const unsigned char *iv)
{
    ...
    for (unsigned int i = 0; i < inPaddedLen; i += blockBytesLen)
    {
        XorBlocks(block, inPadded + i, block, blockBytesLen);
...
unsigned char *AES::DecryptCBC(const unsigned char in[], unsigned int inLen, const unsigned char key[],
                               const unsigned char *iv)
{
    CheckLength(inLen);
    unsigned char *out = new unsigned char[inLen];
...
unsigned char *AES::EncryptCFB(const unsigned char in[], unsigned int inLen, const unsigned char key[],
                               const unsigned char *iv)
{
    unsigned char *inPadded;
    unsigned int inPaddedLen;
    ...
    for (unsigned int i = 0; i < inPaddedLen; i += blockBytesLen)
    {
        ...
        XorBlocks(inPadded + i, encryptedBlock, out + i, blockBytesLen);
        memcpy(block, out + i, blockBytesLen);
    }
...
unsigned char *AES::DecryptCFB(const unsigned char in[], unsigned int inLen, const unsigned char key[],
                               const unsigned char *iv)
{
    CheckLength(inLen);
    unsigned char *out = new unsigned char[inLen];
...
std::vector<unsigned char> AES::EncryptECB(std::vector<unsigned char> in, std::vector<unsigned char> key)
{
    ...
    size_t size                  = in.size() + blockBytesLen - in.size() % blockBytesLen;
    std::vector<unsigned char> v = ArrayToVector(out, size);
    delete[] out;
    return v;
}
...
std::vector<unsigned char> AES::EncryptCBC(std::vector<unsigned char> in, std::vector<unsigned char> key,
...
std::vector<unsigned char> AES::DecryptCBC(std::vector<unsigned char> in, std::vector<unsigned char> key,
                                           std::vector<unsigned char> iv)
{
...
std::vector<unsigned char> AES::EncryptCFB(std::vector<unsigned char> in, std::vector<unsigned char> key,
                                           std::vector<unsigned char> iv)
{
...
std::vector<unsigned char> AES::DecryptCFB(std::vector<unsigned char> in, std::vector<unsigned char> key,
                                           std::vector<unsigned char> iv)
{
