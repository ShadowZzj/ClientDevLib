...
pOpenCharacterMappingFile(const char *szLeafname)
{
#if !defined(__riscos)
	FILE	*pFile;
    const char	*szHome, *szAntiword, *szSuffix, *szKepmDataDir;
	size_t	tFilenameLen;
    ...
    szKepmDataDir = szGetKepmDataDirectory();
    if (strlen(szKepmDataDir) + tFilenameLen <
        sizeof(szMappingFile) -
        sizeof(ANTIWORD_DIR) -
        2 * sizeof(FILE_SEPARATOR)) {
        sprintf(szMappingFile,
            "%s" FILE_SEPARATOR "%s%s",
                szKepmDataDir, szLeafname, szSuffix);
        DBG_MSG(szMappingFile);
