...
if(WIN32)
    ...
    function(ClientDevLibFinalize)
        # 选择一个可用的运行时输出目录（某些构建类型下 RELWITHDEBINFO 可能为空）
        set(_runtime_out "")
        if(DEFINED CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELWITHDEBINFO AND NOT "${CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELWITHDEBINFO}" STREQUAL "")
            set(_runtime_out "${CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELWITHDEBINFO}")
        elseif(DEFINED CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG AND NOT "${CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG}" STREQUAL "")
            set(_runtime_out "${CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG}")
        elseif(DEFINED CMAKE_RUNTIME_OUTPUT_DIRECTORY AND NOT "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}" STREQUAL "")
            set(_runtime_out "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}")
        else()
            # 最后兜底：至少别让 file(COPY) 因 DESTINATION 为空而失败
            set(_runtime_out "${CMAKE_BINARY_DIR}")
        endif()

        foreach(_file ${WINDOWS_UTIL_DLL_FILES})
            message(STATUS "Copying ${_file} to ${_runtime_out}")
            ...
            if(${_file} MATCHES ".*General/ThirdParty/python/.*win/x86$")
                ...
                if(EXISTS ${_runtime_out}/pythonlib)
                    file(REMOVE_RECURSE ${_runtime_out}/pythonlib)
                endif()
                file(COPY ${_file} DESTINATION ${_runtime_out})
                # rename the folder to pythonlib
                file(RENAME ${_runtime_out}/x86 ${_runtime_out}/pythonlib)
            elseif(${_file} MATCHES ".*General/ThirdParty/python/.*win/x64$")
                ...
                file(COPY ${_file} DESTINATION ${_runtime_out})
                # rename the folder to pythonlib
                file(RENAME ${_runtime_out}/x64 ${_runtime_out}/pythonlib)
            else()
                file(COPY ${_file} DESTINATION ${_runtime_out})
            endif()
        endforeach()
