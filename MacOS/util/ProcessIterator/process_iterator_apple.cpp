...
int unique_nonzero_ints(int* arr_in, int len_in, int* arr_out) {
	...
	if (arr_in == arr_out) {
		source = (int*)malloc(sizeof(int)*len_in);
...
int init_process_iterator(process_iterator *it, process_filter *filter){
	it->i = 0;
...
int get_next_process(process_iterator *it, process *p){
